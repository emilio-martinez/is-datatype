...
describe(`isOneOfMultipleTypes`, () => {
  ...
  it(`should only take valid \`DataType\` values for the \`type\` argument`, () => {
    invalidTypeValues.forEach( n =>
      expect( isOneOfMultipleTypes(true, (n as DataType)) )
        .toBe(false, `Failed test for ${JSON.stringify(n)}`) )
  });
  ...
  it(`should immediately return \`true\` when \`any\` is passed`, () => {
    aggregateUseCases
      .forEach( n =>
        n.forEach( m => {
          expect( isOneOfMultipleTypes(m, DataType.any) )
            .toBe(true, `Failed for \`${m}\` of type \`${typeof m}\``);
          expect( isOneOfMultipleTypes(m, [ DataType.any ]) )
            .toBe(true, `Failed for \`${m}\` of type \`${typeof m}\``);
        }));
  });
  ...
  it(`should test multiple \`DataType\` in addition to a single one`, () => {
    ...
    testCases
      .forEach( n =>
        n.test.forEach( m =>
          expect( isOneOfMultipleTypes(m, n.type) )
            .toBe(n.expect, `Failed for ${m} when \`type\` is ${JSON.stringify(n.type)}`) ) );
  });
